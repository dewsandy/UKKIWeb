<div class="col-md-12 graphs">
  ...
  if(isset($_GET['cari'])){
    if($jhal != 1){
      if($pagenum > 1){
        $prev = $pagenum - 1;
        $pag = '<a  class="page" href="berita&cari='.$_GET['cari'].'&hal='.$prev.'">Previous</a>';
        for($i = $pagenum - 4; $i<$pagenum; $i++)
        {
          if($i > 0)
          {
            $pag = '<a class="page" href="berita&cari='.$_GET['cari'].'&hal='.$i.'">'.$i.'</a>';
          }
        }
      }
      $pag = '<a class="page active" href="berita&cari='.$_GET['cari'].'&hal='.$pagenum.'">'.$pagenum.'</a>';
      ...
      {
        $next = $pagenum + 1;
        $pag = '<a class="page" href=berita&cari='.$_GET['cari'].'&hal='.$next.'">next</a>';
      }
    }
  }
  else
  {
    if($jhal != 1){
      if($pagenum > 1){
        $prev = $pagenum - 1;
        $pag = '<a  class="page" href="berita&hal='.$prev.'">Previous</a>';
        for($i = $pagenum - 4; $i<$pagenum; $i++)
        {
          if($i > 0)
          {
            $pag = '<a class="page" href="berita&hal='.$i.'">'.$i.'</a>';
          }
        }
      }
      $pag = '<a class="page active" href="berita&hal='.$pagenum.'">'.$pagenum.'</a>';
      for($i= $pagenum+1; $i<= $jhal; $i++)
      {
        $pag = '<a class="page" href="berita&hal='.$i.'">'.$i.'</a>';
        if($i >= $pagenum - 4)
      ...
      {
        $next = $pagenum + 1;
        $pag = '<a class="page" href=berita&hal='.$next.'">next</a>';
      }
    }
  }
...
tblberita INNER JOIN tblkategori ON tblkategori.idkategori=tblberita.kategori_id INNER JOIN tbladmin ON tbladmin.idadmin = tblberita.iduser
  ...
  ?>
  <a type='button' href="inberita" class='btn btn-warning'>Tambah Data</a> 
<?php
    ...
    <div class="table-responsive">
      <table class="table">
        ...
        <tbody>
         <?php
            $no = 0;
            while($rb = $berita -> fetch_assoc()){
              $no++;
            if($rb['status'] == 1){
                $idberita =$rb['idberita'];
                 if(strlen($rb['isi']) >= 200){
                             $isi = substr($rb['isi'],0,200);
                    ...
                    <td>".$rb['kategori']."</td>
                     ...
                     <td>".$rb['nama']."</td>
                      <td>".$rb['in_tgl']."</td>
                       <td>".$rb['edit_tgl']."</td>
                    <td>
                      <button type='button' onclick=\"window.location.href='upberita&id=$idberita';\" class='btn btn-warning'>Ubah</button> 
                      <button type='button' class='btn btn-danger hapusberita' id='$idberita'>Hapus</button>&nbsp;
                    </td> 
                  </tr>
                ";
...
<script language="javascript">
    $(document).ready(function()
  {
      $('.hapusberita').click(function()
      {
        ...
        if(answer)
        {
          $.post(url, {hapusberita: idberita}, function(){
