...
tblinfo INNER JOIN tblkategori ON tblkategori.idkategori=tblinfo.kategori_id INNER JOIN tbladmin ON tbladmin.idadmin = tblinfo.iduser
   WHERE tblkategori.idkategori=1 ORDER BY tblinfo.edit_tgl ASC";
  ...
  $pag = '';
    ...
    if($jhal != 1){
      if($pagenum > 1){
        $prev = $pagenum - 1;
        $pag = '<li><a href="infointernal&hal='.$prev.'">&laquo;</a></li>';
        for($i = $pagenum - 4; $i<$pagenum; $i++)
        {
          if($i > 0)
          {
            $pag = '<li><a href="infointernal&hal='.$i.'">'.$i.'</a></li>';
          }
        }
      }
      $pag = '<li><a class="active" href="infointernal&hal='.$pagenum.'">'.$pagenum.'</a></li>';
      ...
      {
        $next = $pagenum + 1;
        $pag = '<li><a href="infointernal&hal='.$next.'">&raquo;</a></li>';
      }
    }
...
tblinfo INNER JOIN tblkategori ON tblkategori.idkategori=tblinfo.kategori_id INNER JOIN tbladmin ON tbladmin.idadmin = tblinfo.iduser
   WHERE tblkategori.idkategori=1 ORDER BY tblinfo.edit_tgl ASC LIMIT $mulai,$batas");
